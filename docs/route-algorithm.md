...
```
Per-scenario cache → Eligibility gate → Offroad check → GT candidates → DP sequence search → Dead-end extension
```
...
`_can_compute_route()` filters non-ego agents before any geometric work:
- The agent must be valid at `--route_check_timestep`
- From that timestep onwards it must have at least `--min_route_valid_points` percent of valid samples
- It must not be offroad at the check timestep

Ego (vehicle id 0) bypasses this gate; if its route ends up empty, conversion fails.

### 3. Offroad Check

`_is_offroad_at_timestep()` keeps the current agent selection logic:
- If the agent's position is farther than 5m from all lane centerlines, or 0.3m when nearly stationary, skip it
- If the agent's footprint intersects a road edge boundary, skip it

### 4. GT Lane Candidates
