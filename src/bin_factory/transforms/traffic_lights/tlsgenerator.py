...
from src.bin_factory.transforms.traffic_lights.utils import TLS, ApproachingLane, Direction, UnionFind
...
class TLSGenerator:
    ...
    def _get_feasible_states(self) -> list[list[dict[tuple, TLS]]]:
        ...
        if len(self.container_template) == 4:
            ...
            for j in range(3):
                for phase in candidate_state[j]:
                    candidate_state[j][phase] = TLS.RED if j == 2 else TLS.GREEN
            candidate_states.append(candidate_state)
        else:
            assert False
